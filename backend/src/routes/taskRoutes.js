...
router.post(
  ...
  async (req, res) => {
    ...
    const project = await db.prepare('SELECT * FROM projects WHERE id = ? AND user_id = ?').get(projectId, req.user.id);
    if (!project) {
      return sendError(res, 404, 'Projeto não encontrado.');
    }

    if (assigneeId) {
      const member = await db.prepare('SELECT * FROM project_members WHERE project_id = ? AND user_id = ?').get(projectId, assigneeId);
      if (!member) {
    ...
    const task = await db.prepare('SELECT t.*, p.name as project_name, u.name as assignee_name FROM tasks t LEFT JOIN projects p ON p.id = t.project_id LEFT JOIN users u ON u.id = t.assignee_id WHERE t.id = ?').get(result.lastInsertRowid);
...
router.put(
  ...
  async (req, res) => {
    ...
    if (assigneeId) {
      const member = await db.prepare('SELECT * FROM project_members WHERE project_id = ? AND user_id = ?').get(task.project_id, assigneeId);
      if (!member) {
        return sendError(res, 400, 'Usuário não pertence a este projeto.');
      }
    }
    ...
    const updated = await db.prepare('SELECT t.*, p.name as project_name, u.name as assignee_name FROM tasks t LEFT JOIN projects p ON p.id = t.project_id LEFT JOIN users u ON u.id = t.assignee_id WHERE t.id = ?').get(req.params.id);
...
router.delete('/:id', async (req, res) => {
  ...
  await db.prepare('DELETE FROM tasks WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
