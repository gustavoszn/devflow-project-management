...
router.get('/', (req, res) => {
  ...
  const data = projects.map((project) => ({
    ...project,
    progress: buildProjectProgress(project.id),
    taskCount: db.prepare('SELECT COUNT(*) as total FROM tasks WHERE project_id = ?').get(project.id).total,
...
router.post(
  ...
  (req, res) => {
    ...
    return sendSuccess(res, 201, { ...project, progress: 0, taskCount: 0, members: getProjectMembers(projectId) }, 'Projeto criado com sucesso.');
  }
);
...
router.put(
  ...
  (req, res) => {
    ...
    return sendSuccess(res, 200, { ...updated, progress: buildProjectProgress(updated.id), members: getProjectMembers(updated.id) }, 'Projeto atualizado com sucesso.');
  }
);
